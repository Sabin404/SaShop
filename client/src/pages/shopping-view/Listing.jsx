...
const Listing = () => {
  ...
  return (
    <div className='grid grid-cols-1 md:grid-cols-[300px_1fr] gap-5 p-4 md:p-6 '>
      ...
      <div className="bg-background w-full rounded-lg shadow-sm z-10">
          ...
          <div className="flex items-center gap-2">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuPortal>
                <DropdownMenuContent
                  align="end"
                  className="w-[200px] rounded-2xl bg-white z-[9999] shadow-lg border border-gray-200 p-2 font-semibold"
                >
                  <DropdownMenuRadioGroup value={sort} onValueChange={handleSort}>
                    {sortOptions.map(sortItem => (
                      <DropdownMenuRadioItem
                        key={sortItem.id}
                        value={sortItem.id}
                        className="px-3 py-2 rounded-md hover:bg-blue-100 focus:bg-blue-200 cursor-pointer transition-colors text-gray-800 text-sm"
                      >
                        {sortItem.label}
                      </DropdownMenuRadioItem>
                    ))}
